//Akan dibuat setelah jadi database handler
import 'package:project_working_class/models/send_absen.dart';
...
class DatabaseHandler {
  ...
  Future<int> insertUser(List<SendAbsen> users) async {
    ...
    for(var user in users){
      ...
      result = await db.insert('report', user.toJson());
    }
    return result;
  }
  ...
  Future<int> retrievePending() async {
    final Database db = await initializeDB();
    final List<Map<String, Object>> queryResult = await db.query('report', where: "SEND = ", whereArgs: ["pending"]);
