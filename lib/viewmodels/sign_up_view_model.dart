  ...
  void register(BuildContext context) async {
    ...
    try{
      if (nameController.text.length != null &&
        ...
        if (data.code == 200){
          setBusy(false);
          _alertService.showAlert(context, "Berhasil", "Registrasi Berhasil", (){
            _navigationService.replaceTo(LoginViewRoute);
          });
        } else {
          setBusy(false);
          _alertService.showAlert(context, 'Kesalahan', 'Ada yang salah ${data.message}', _navigationService.pop);
        }
      } else {
        setBusy(false);
        _alertService.showAlert(context, 'Peringatan', 'Tolong isi kelengkapan data', _navigationService.pop);
      }
    }catch (e){
      _alertService.showAlert(context, 'Kesalahan', 'Periksa kode registrasi anda', _navigationService.pop);
    }
  }
